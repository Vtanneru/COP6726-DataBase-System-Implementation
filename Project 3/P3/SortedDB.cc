...
int SortDB::Create (char *f_path, fType f_type, void *startup) {
...
void SortDB::Load (Schema &f_schema, char *loadpath) {
...
int SortDB::GetNext (Record &fetchme) {
...
int SortDB::GetNext (Record &fetchme, CNF &cnf, Record &literal) {
    if(!boundCalculated) {
        boundCalculated = 1;
        set<int> attSet;
...
void SortDB::writeMode(){
    ...
    if(isWriting==0) {
        ...
        pthread_create(thread, NULL, Work_func , (void *) workerArg);
        cout << "end writeMode " << endl;
    }
}

void SortDB::readMode(){
    if(isWriting==1){
        boundCalculated = 0;
        isWriting = 0;
        in->ShutDown();
...
int SortDB :: Run (Record *left, Record *literal, Comparison *c) {
